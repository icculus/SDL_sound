...
static void do_song_load(SDL_IOStream *rw, SDL_AudioSpec *audio, MidiSong **out)
{
  ...
  *out = NULL;
  ...
  int buffer_size = 4096; // @TODO was "audio->samples"
  song->buffer_size = buffer_size;
  song->resample_buffer = SDL_malloc(buffer_size * sizeof(sample_t));
  if (!song->resample_buffer) goto fail;
  song->common_buffer = SDL_malloc(buffer_size * 2 * sizeof(Sint32));
