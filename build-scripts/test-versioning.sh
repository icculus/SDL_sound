...
if [ "$ref_version" = "$version" ]; then
    ...
    not_ok "CMakeLists.txt $version disagrees with SDL_sound.h $ref_version"
fi
...
for rcfile in src/version.rc; do
    ...
    if [ "$ref_tuple" = "$tuple" ]; then
        ...
        not_ok "$rcfile FILEVERSION $tuple disagrees with SDL_sound.h $ref_tuple"
    fi
    ...
    if [ "$ref_tuple" = "$tuple" ]; then
        ...
        not_ok "$rcfile PRODUCTVERSION $tuple disagrees with SDL_sound.h $ref_tuple"
    fi
...
# else
#     not_ok "Info-Framework.plist CFBundleShortVersionString $version disagrees with SDL_sound.h $ref_version"
# fi
...
# else
#     not_ok "Info-Framework.plist CFBundleVersion $version disagrees with SDL_sound.h $ref_version"
# fi

# For simplicity this assumes we'll never break ABI before SDL 3.
# dylib_compat=$(sed -Ene 's/.*DYLIB_COMPATIBILITY_VERSION = (.*);$/\1/p' Xcode/SDL_sound.xcodeproj/project.pbxproj)
...
# dylib_cur=$(sed -Ene 's/.*DYLIB_CURRENT_VERSION = (.*);$/\1/p' Xcode/SDL_sound.xcodeproj/project.pbxproj)
